...
Tag.parse = function(oRes){
  ...
  for (var x=0;x<a.length;x++){
    var tag = a[x];
    Tag.dict[(tag.id).toLowerCase()] = "" + tag;
  }
...
Tag.tag = function(id) {
  ...
  var sTag = Tag.dict[id.toLowerCase()];
  if (sTag == undefined) {
...
Tag.callJs = function(tag) {
  ...
  try{
    window[Tag.jsfunction](tag);
    trace("Ok tag -> " + Tag.jsfunction + "('" + tag + "')");
  }catch (e){
    trace("Ko tag chiamata alla funzione -> '" + Tag.jsfunction + "' che non è definita.");
  }
